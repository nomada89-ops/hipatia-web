...
const MainFormSecure: React.FC<MainFormProps> = ({ onBack, userToken }) => {
    ...
    const processAndAddFiles = async (files: File[]) => {
        // First optimize images to reduce size for display and potential network usage
        ...
        try {
            for (const file of files) {
                if (file.type.startsWith('image/')) {
                    const optimized = await optimizeExamImage(file);
            ...
            alert("Error optimizando algunas imágenes. Se usarán los originales.");
            optimizedFiles.push(...files);
        } finally {
            setIsOptimizing(false);
        }

        // Now update state with files for display
        setExamenArchivos(prev => [...prev, ...optimizedFiles]);

        // NOW: Run Sequential OCR + Anonymization
